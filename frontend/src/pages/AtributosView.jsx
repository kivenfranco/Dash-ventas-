...
const SEG_OPTS = [
  { key: 'es_stock',        label: 'Stock vs No Stock' },
  { key: 'estructura',      label: 'Estructura' },
  { key: 'dispositivo',     label: 'Válvula/Dispositivo' },
  { key: 'tipo_producto',   label: 'Tipo Producto' },
  { key: 'tipo_fabricacion',label: 'Fabricación' },
]
...
export function AtributosView() {
  ...
  const [seg, setSeg]  = useState('es_stock')

  const { data, loading, error } = useData(() => api.atributos(filters, seg, 20), [filters, refreshKey, seg])
  ...
  const chartData = rows.slice(0, 15).map((d) => ({
  ...
  const segLabel = SEG_OPTS.find((s) => s.key === seg)?.label

  if (error) return (
    <div className="flex items-center justify-center h-64">
      <div className="text-center">
        <p className="text-red-400 font-semibold mb-1">Error cargando datos</p>
  ...
  return (
    <div className="flex flex-col gap-5 animate-fade-in">
      <div className="flex items-center justify-between">
        <div>
          <h1 className="text-xl font-bold text-slate-100">Ventas por Atributo de Producto</h1>
          <p className="text-slate-500 text-xs mt-0.5">Análisis por características del producto · {period}</p>
        </div>
      ...
      <div className="flex flex-wrap gap-3">
        ...
        <Chip label="Categorías"      value={rows.length}        color="text-cyan-400" />
      </div>

      <div className="grid grid-cols-1 xl:grid-cols-2 gap-4">
        {/* Bar chart */}
        <div className="card">
          <h2 className="text-sm font-semibold text-slate-200 mb-0.5">Ventas por {segLabel}</h2>
          <p className="text-xs text-slate-500 mb-4">actual vs año anterior</p>
        ...
        <div className="card">
          ...
          <p className="text-xs text-slate-500 mb-4">% ventas netas por {segLabel}</p>
          <div className={`h-72 ${loading ? 'opacity-40 animate-pulse' : ''}`}>
            <ResponsiveContainer width="100%" height="100%">
              <PieChart>
                ...
                <Tooltip formatter={(v, n) => [fmtCOP(v), n]} contentStyle={{ background: '#161b27', border: '1px solid #1f2937', borderRadius: 8, fontSize: 11 }} />
      ...
      {/* Detail table */}
      <div className="card">
        <h2 className="text-sm font-semibold text-slate-200 mb-4">Tabla — {segLabel}</h2>
        <div className="overflow-x-auto">
          <table className="w-full text-xs">
            <thead>
              <tr className="text-left border-b border-surface-700 text-slate-400">
                <th className="pb-3 font-medium">#</th>
                <th className="pb-3 font-medium">{segLabel}</th>
              ...
              {loading
                ...
                : rows.map((d, i) => (
                  <tr key={i} className="border-b border-surface-700/30 hover:bg-surface-700/20 transition-colors">
                    ...
                    <td className="py-2.5 text-right text-slate-400">{fmtInt(d.num_productos)}</td>
                  </tr>
                ))
              }
              {rows.length > 0 && (
