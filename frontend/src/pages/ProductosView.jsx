...
export function ProductosView() {
  ...
  const pieData  = rows.slice(0, 8).map((d, i) => ({ name: d.dimension, value: d.ventas_netas, fill: PALETTE[i % PALETTE.length] }))
    ...
    <div className="flex flex-col gap-5 animate-fade-in">
      ...
      <div className="grid grid-cols-1 xl:grid-cols-5 gap-4">
        <div className="xl:col-span-3 card">
          <h2 className="text-sm font-semibold text-slate-200 mb-0.5">Ventas vs Año Anterior</h2>
          ...
          <div className={`h-64 ${loading ? 'opacity-40 animate-pulse' : ''}`}>
            <ResponsiveContainer width="100%" height="100%">
              <BarChart data={barData} layout="vertical" margin={{ top: 4, right: 16, left: 8, bottom: 4 }}>
                ...
                <Bar dataKey="ventas_netas" name="Ventas Netas" fill="#6366f1" radius={[0, 3, 3, 0]} />
                <Bar dataKey="ventas_netas_ant" name="Año Anterior" fill="#1f2937" stroke="#475569" strokeWidth={1} radius={[0, 3, 3, 0]} />
              </BarChart>
            </ResponsiveContainer>
          </div>
        </div>
