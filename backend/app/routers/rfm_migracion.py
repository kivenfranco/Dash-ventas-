...
def _period_rfm_sql(
    ...
    The subquery is aliased so callers can wrap it.
    """
    cond_parts = [f"fv.ANO_FISCAL = {ano}"]

    if mes:
        cond_parts.append(f"fv.PERIODO_FISCAL <= {mes}")
    elif ano == date.today().year:
        cond_parts.append(f"fv.PERIODO_FISCAL <= {date.today().month}")
...
def get_rfm_migracion(
    ...
    cache_key = f"rfm_migracion:{ano}:{mes}:{excl_pvta}:{top_n}"
    if (hit := cache.get(cache_key)):
        return hit

    sql_cur  = _period_rfm_sql(cfg, ano,      mes, excl_pvta, top_n)
    sql_prev = _period_rfm_sql(cfg, ano_prev, mes, excl_pvta, top_n)
